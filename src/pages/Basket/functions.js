...
export const areProductsEqual = (newProduct, products) => {
  const filteredProducts = products.filter(
    (product) =>
      product.id === newProduct.id && product.type === newProduct.type,
  );
  if (!filteredProducts.length) {
    return { equal: false };
  }
  const {
    removed: newRemovedList,
    added: newAddedList,
    dough: newDough,
...
export const convertPizzaProduct = (pizza) => {
  ...
  convertedPizza.info += ` ${
    pizza.diameters.find((diameter) => diameter.selected).title
  }`;
  convertedPizza.added = pizza.optionalIngredients.filter(
    (ingredient) => ingredient.selected,
  );
  convertedPizza.removed = pizza.ingredients.filter(
    (ingredient) => !ingredient.selected,
  );
...
export const getPizzaKey = (pizza) => {
  const { id, count, type, removed = [], added = [], dough, diameters } = pizza;
  const removedKey = removed
    .map((element) => `${element.id}${element.selected}`)
    .join('');
  const addedKey = added
    .map((element) => `${element.id}${element.selected}`)
    .join('');
  const doughKey = dough
    .map((element) => `${element.id}${element.selected}`)
    .join('');
  const diametersKey = diameters
    .map((element) => `${element.id}${element.selected}`)
    .join('');
