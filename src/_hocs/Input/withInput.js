...
const withInput = (Component) => {
  return ({ name, id, label, initialValue, Dropdown, Icon, ...otherProps }) => {
    const { required, phone, wrapperClassName, type, readOnly, min, onChange } =
      otherProps;
    const [isError, setIsError] = useState(false);
    const labelClassName = `${styles.input__label}${
      isError ? ` ${styles.input__label_error}` : ''
    }`;
    const inputClassName = useMemo(() => {
      ...
      return additionalClasses.length
        ? getClassName('', additionalClasses)
        : '';
    }, [isError, readOnly]);

    const blockClassName = useMemo(() => {
      return getClassName(styles.input__block, wrapperClassName);
    }, [wrapperClassName]);
    const input = useRef();
