...
const withSlider = (Component, { elements, count = 0, listClassName, elementClassName, mainClassName }) => {
  return ({ ...props }) => {
    ...
    return (
      <div className={mainWrapperClassName} style={{ width: `${wrapperSliderWidth}px`, height: `${sliderHeight}px` }}>
        ...
        <div className={wrapperClassName} style={{ width: `${sliderWidth}px`, height: `${sliderHeight.current}px` }}>
          { elements.map((element, idx) => 
            <div key={`${element.id}${idx}`} ref={ref} className={elClassName} style={{ left: `${(idx + leftCounter) * elementWidth}px` }}>
